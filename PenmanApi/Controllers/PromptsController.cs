    ...
    public class PromptsController : ControllerBase
    {
        ...
        public IActionResult Create([FromBody]CreatePromptDto promptDto)
        {
            ...
            catch (CollisionException)
            {
                return BadRequest(new ErrorResponseDto
                {
                    ...
                    DisplayErrorMessage = "The API encountered an unexpected error, but the failed request will automically retry.",
        ...
        public IActionResult ReadAll([FromQuery]ReadAllPromptsDto readAllPromptsDto)
        {
            try
            {
                ...
                var lastReadAllDate = readAllPromptsDto.LastReadAllDate.HasValue ? readAllPromptsDto.LastReadAllDate.Value : new DateTime(1970, 1, 1);
                var lastReadAllResponse = DateTime.Now;
                var prompts = _promptService.ReadAll(authenticatedUserId, readAllPromptsDto.UserId, lastReadAllDate);
                var responseDto = new PromptCollectionResponseDto
                {
                    Prompts = prompts.Select(p => _mapper.Map<PromptResponseDto>(p)).ToArray(),
                    LastReadAllDate = lastReadAllResponse,
                };
                return Ok(responseDto);
            }
            catch (Exception ex)
        ...
        public IActionResult Update([FromBody]UpdatePromptDto promptDto)
        {
            try
            {
                ...
                var promptEntity = _promptService.UpdatePrompt(
                    authenticatedUserId,
                    promptDto.PromptId,
                    promptDto.EventEndDate,
                    promptDto.EventEndDate,
                    promptDto.Title,
                    promptDto.Body
                );
                var responseDto = _mapper.Map<PromptResponseDto>(promptDto);
                return Ok(responseDto);
            }
            catch (Exception ex)
            {
