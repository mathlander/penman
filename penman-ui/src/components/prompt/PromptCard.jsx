...
function PromptCard(props) {
    ...
    const [eventStartDatePickers, setEventStartDatePickers] = useState([]);
    const [eventEndDatePickers, setEventEndDatePickers] = useState([]);
    ...
    const handleDeletePrompt = () => {
        ...
        dispatch(deleteEntity(props.storageManager, props.authenticatedUser.toReplayUser(), props.prompt, props.isOffline));
    };
    ...
    const handleUpdate = (e) => {
        ...
        dispatch(update(props.storageManager, props.authenticatedUser.toReplayUser(), props.prompt, props.isOffline));
        setIsEditing(false);
    };
    useEffect(() => {
        if (eventStartDatePickers.length === 0) {
            const eventStartDateInputElement = document.getElementById(`prompt-event-start-${timestamp}`);
            const eventEndDateInputElement = document.getElementById(`prompt-event-end-${timestamp}`);
            ...
            setEventStartDatePickers(M.Datepicker.init(eventStartDateNodeList, {
                ...
                onSelect: (selectedDate) => props.prompt.eventStartDate = selectedDate,
            }));
            setEventEndDatePickers(M.Datepicker.init(eventEndDateNodeList, {
                defaultDate: props.prompt.eventEndDate || now,
                onSelect: (selectedDate) => props.prompt.eventEndDate = selectedDate,
            }));
    ...
    }, [eventStartDatePickers, eventEndDatePickers, props.prompt.eventEndDate, props.prompt.eventStartDate, timestamp]);
    return (
